          ...
          <b-col sm="6">
            <b-form class="form text-left" @submit.stop.prevent @submit="onSubmit">
                ...
                <b-button type="submit" style="margin-top: 20px" size="sm" class="mb-2">
                    <b-icon icon="plus-circle" aria-hidden="true"></b-icon> Add tag
                </b-button>
            </b-form>
          </b-col>
          <b-col sm="6">
...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      if (this.errors.length == 0) {
        ...
        let response = await postExternalRequest('tags/', tagName)
  ...
  async mounted() {
    let response = await getExternalRequest('attributes/')
    if (response.status == 200){
